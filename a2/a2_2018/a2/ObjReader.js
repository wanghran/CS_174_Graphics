...
function initVertexBuffers(gl, program) {
    ...
    o.vertexBuffer = createEmptyArrayBuffer(gl, 3, gl.FLOAT);
    o.normalBuffer = createEmptyArrayBuffer(gl, 3, gl.FLOAT);
    o.colorBuffer = createEmptyArrayBuffer(gl, 4, gl.FLOAT);
...
function createEmptyArrayBuffer(gl, num, type) {
...
// Read a file
function readOBJFile(fileName, scale, reverse, handle_complete) {
    var request = new XMLHttpRequest();

    request.onreadystatechange = function () {
        if (request.readyState === 4 && request.status !== 404) {
            onReadOBJFile(request.responseText, fileName, scale, reverse, handle_complete);
        }
    }
...
function onReadOBJFile(fileString, fileName, scale, reverse, handle_complete) {
...
var OBJDoc = function (fileName) {
    ...
    this.texcoords = new Array(0); // Initialize the property for Tex Coords
}
...
OBJDoc.prototype.parse = function (fileString, scale, reverse) {
    ...
    var currentObject = new OBJObject("default");
    ...
    while ((line = lines[index++]) != null) {
        ...
        switch (command) {
            ...
            case 'vn':   // Read normal
                ...
                continue; // Go to the next line
            case 'vt':   // Read normal
                var tex_coord = this.parseTexCoord(sp);
                this.texcoords.push(tex_coord);
                continue; // Go to the next line
...
OBJDoc.prototype.getDrawingInfo = function () {
        ...
        for (var j = 0; j < object.faces.length; j++) {
            ...
            for (var k = 0; k < face.vIndices.length; k++) {
                ...
                if (face.tIndices.length > 0)
                {
                    ...
                    var tex_coord = this.texcoords[tIdx];
                    texcoords[index_indices * 2 + 0] = tex_coord.u;
                    texcoords[index_indices * 2 + 1] = tex_coord.v;
                }
    ...
    return new DrawingInfo(vertices, normals, texcoords, colors, indices);
}
...
// DrawInfo Object
//------------------------------------------------------------------------------
var DrawingInfo = function (vertices, normals, texcoords, colors, indices) {
